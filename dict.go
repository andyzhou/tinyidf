...
// GetFreq returns the freq of the given word by d.freqMap[word].
func (d *Dictionary) GetFreq(word string) (int64, bool) {
	freq, ok := d.freqMap[word]
	return freq, ok
}
...
func NewIDFTable(file string) *IDFTable {
	...
	for {
		...
		idf.freqMap[word] = freq
	}

	freqs := make([]float64, 0, len(idf.freqMap))
	for _, freq := range idf.freqMap {
...
// GetFreq returns the freq of the given word by idf.freqMap[word].
func (idf *IDFTable) GetFreq(word string) (float64, bool) {
	freq, ok := idf.freqMap[word]
	return freq, ok
}